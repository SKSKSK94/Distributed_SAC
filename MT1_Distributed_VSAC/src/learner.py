...
class Learner():
    ...
    def set_cfg_parameters(self, save_period, write_mode):
        ...
        self.print_period = int(self.cfg['print_period_learner'])
        self.update_delay = self.cfg['update_delay']
    ...
    def build_model(self):
        ...
        self.local_critic_1 = Critic(self.state_dim, self.action_dim, self.device, hidden_dim=self.critic_hidden_dim)
        self.local_critic_2 = Critic(self.state_dim, self.action_dim, self.device, hidden_dim=self.critic_hidden_dim)
        self.target_critic_1 = Critic(self.state_dim, self.action_dim, self.device, hidden_dim=self.critic_hidden_dim)
        self.target_critic_2 = Critic(self.state_dim, self.action_dim, self.device, hidden_dim=self.critic_hidden_dim)
    ...
    def save_checkpoint(self, episode_idx):
        ...
        torch.save(state, self.save_model_path+'checkpoint_{}.tar'.format(str(episode_idx))) 
    ...
    def to_device(self):        
        ...
        self.local_critic_2.to(self.device)
        self.target_critic_1.to(self.device)
        self.target_critic_2.to(self.device)

    def push_log(self, update_iteration, critic_loss, actor_loss):
    ...
    def update(self):
        ...
        critic_loss, actor_loss = self.update_SAC(
            ...
            dones, 
            alpha, 
            retain_graph=False #### for CARE, retain_graph=True
        )
        return critic_loss, actor_loss
