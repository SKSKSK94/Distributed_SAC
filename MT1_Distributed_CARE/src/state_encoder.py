...
class stateEncoder(nn.Module):
    ...
    def __init__(self, encoder_cfg):        
        ...
        self.trunk = build_mlp(
            input_dim=int(encoder_cfg['embedding_dim_contextEnc']),
    ...
    def forward(self, z_context : torch.Tensor, mtobss : torch.Tensor, detach_z_encs=False):
        ...
        assert z_context_dim == z_enc_dim, 'z_context_dim should be same as z_enc_dim for calculating attention.'
...
class Linear(nn.Module):
    def __init__(self,
        ...
        self.W = nn.Parameter(torch.randn(num_encoders, in_features, out_features), requires_grad=True)
        self.b = nn.Parameter(torch.randn(num_encoders, 1, out_features), requires_grad=True)
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 2:
            ...
            assert in_features == self.in_features, 'Error has occured at linear layer for parallel computation of `Mixture of Encoders`'            
            return torch.einsum('kio,bi->kbo', self.W, x) + self.b 
        elif len(x.shape) == 3:
            num_encoders, _, in_features = x.shape
            assert num_encoders == self.num_encoders, 'Error has occured at linear layer for parallel computation of `Mixture of Encoders`'
            assert in_features == self.in_features, 'Error has occured at linear layer for parallel computation of `Mixture of Encoders`'
