...
class Player():
    ...
    def set_cfg_parameters(self):
        ...
        self.max_episode_time = self.cfg['max_episode_time'] # maximum episode time for the given environment

    def build_model(self):
        self.context_encoder = contextEncoder(self.encoder_cfg)
        self.actor = Actor(self.cfg['actor'], self.encoder_cfg)
    ...
    def trajectory_generator(self, task_idx):
        ...
        for _ in range(self.env.max_path_length):
            with torch.no_grad():    
                ...
                action = self.actor.get_action(mtobss=mtobs_tensor, z_context=z_context, stochastic=False)
        ...
        while True:        
            ...
            if episode_idx % self.eval_episode_idx == 0:
                for task_idx in self.task_idx_list:
                    ...
                    if self.train_mode:
                        success_rate_data = (self.player_id, self.task_total_step_dict[task_idx], success_rate)
