...
class Learner():
    ...
    def set_cfg_parameters(self, save_period, write_mode):
        ...
        self.log_file = './log/log_MT10_Distributed_CARE/{}/MT10_Distributed_CARE_log_{}.txt'.format(
            used_method_name,
            self.datetime
        )
        ...
        if self.write_mode:
            self.writer = SummaryWriter(
                './log/log_MT10_Distributed_CARE/{}/{}'.format(used_method_name, self.datetime)
            )
    ...
    def update_SAC(self, 
        ...
        Q_loss_1, Q_loss_2 = self.local_critic.cal_loss(
            ...
            action=actions, 
            td_target_values=y,
            use_weighted_loss=self.use_modified_care,
            num_tasks=self.num_tasks,
        ...
        self.soft_update(
            self.local_critic.state_encoder, 
            self.target_critic.state_encoder,
            self.encoder_cfg['state_encoder_tau']    
        )
    ...
    def update(self):
        ...
        #### tie(hard copy) encoders between actor and critic ####
        self.soft_update(self.local_critic.state_encoder, self.actor.state_encoder, tau=1.0)
